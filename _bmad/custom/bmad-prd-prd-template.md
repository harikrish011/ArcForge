...
## 0. Document Purpose
[1 paragraph: who this PRD is for (PM, stakeholders, downstream workflow owners), how it's structured (Glossary-anchored vocabulary, features grouped with FRs nested, assumptions and open ambiguities tagged inline and captured with explicit confidence levels in a dedicated section — never left as narrative prose). If UX work or other inputs already exist, name them here and reference where they live — this PRD builds on them, it does not duplicate.]
...
### 4.1 {Feature Name}
**Description:** [Behavioral narrative — how this feature works, who uses it, the user experience, edge cases. Realizes UJ-X, UJ-Y. Use Glossary terms exactly. Embed inline `[ASSUMPTION: ...]` tags where you inferred without confirmation — each one must have a matching discrete entry in §8 Assumptions & Ambiguities with a confidence level; the inline tag is a reading aid, §8 is the canonical record.]
...
**Notes:** *(optional — `[NOTE FOR PM]` callouts specific to this feature; open questions themselves belong in §8, not here)*
...
*Every inference made without explicit confirmation, and every question still genuinely open — one flat list of discrete, individually tagged items. This is mandatory structure for every PRD (not an optional Adapt-In cluster) and is the canonical record: an inline `[ASSUMPTION: ...]` tag elsewhere in the PRD is a reading aid, not a substitute — every one must have a matching entry here. Never collapse this into narrative prose. Order by confidence ascending (low first) so the items most needing human attention surface at the top.*

- **[ASSUMPTION · confidence: high|medium|low]** (§X.Y) — {what was assumed, stated as a decision the reader can challenge}.
- **[AMBIGUITY · confidence: high|medium|low]** (§X.Y, if tied to a specific section) — {what remains genuinely unresolved, and what it blocks downstream — e.g. "blocks architecture's data model choice"}.
- ...

*Confidence guide — **ASSUMPTION**: `high` = directly implied by explicit source content, stated for the record rather than genuinely in question; `medium` = a reasonable inference, not explicitly stated by the source; `low` = a real guess the author is flagging for priority confirmation before downstream work proceeds on top of it.*
*Confidence guide — **AMBIGUITY**: `high` = a well-scoped, clearly-stated open question with a bounded answer space; `medium` = the shape of the question is clear but the space of plausible answers isn't; `low` = the ambiguity itself may not be fully understood yet — flag it anyway rather than silently resolving it.*
```
